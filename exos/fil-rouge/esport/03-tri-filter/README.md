...
Mais `DataSeries<T>` modifie ce comportement. Deux expériences pour l'observer.
...
Le constructeur de `DataSeries<T>` appelle `ToList()` en interne : la matérialisation est immédiate.
...
var series = DataSeries<ValorantMatch>.From(source);
